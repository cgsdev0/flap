...
float/*FFF*/a,b,c,const   int A=3,                             const                  int B=12,const float
C=60.0,/*FF*/const float  f=16.00,                          const float               e=f/70.0,std::vector<P>
...
(){a=10,b=10,c=0,         I/**/pwx,                 I ch,struct      winsize w;;      ioctl(STDOUT_FILENO,TIOCGWINSZ
,&w/**/                   );srand(                  time(NULL));for(I i=1;i<=ceil     (w.ws_col/C)+2;++i&&pwx=i*c)z.
/*FFF*/                   push_back                 ({i*(I)C,w.ws_row});initscr();    noecho();WINDOW* W=newwin(w.
...
);werase                  (W);float M=1.0;float N= 1.0;a+=N*e;         b+=c*e;c+=0.2  *e;if(c>M)c=
